...
port = 5008             # port number identifies socket on machine
...
def client (name):
    ...
    reply=socket.recv(1024)                     # receive bytes data from listener
    sock.close()                                # up to 1024 bytes in message
    print ('client gor: [%s' %reply)

    if __name__=='__main__':
        from threading import Thread
        sthread = Thread (target=server)
        sthread.deamon = True                   # don't wait for server thread
