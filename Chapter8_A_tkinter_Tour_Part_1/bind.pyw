...
def showPosEvent(event):
    print ('Widget=%s X=%s Y=%s' % (event.widget, event.x , event.y, event.value()))
...
tkroot=Tk()
labelfont = ('curier', 20, 'bold')      # family, size, style
widget =Label(tkroot, text='Hello Bind world')
widget.config(bg='blue', font=labelfont)  #  yellow background, large font
...
tkroot.mainloop()
# moze  se koristiti za kreiranje eventa kao i za kreiranje programa za pracenje kucanje i sta je iskucano
